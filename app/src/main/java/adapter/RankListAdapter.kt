...
class RankListAdapter(private var mData:MutableList<RankComicList.Comic>?= mutableListOf(),private var mcontext:Context) : RecyclerView.Adapter<RankListAdapter.ViewHolder>() {
    ...
    override fun onBindViewHolder(holder: ViewHolder, position: Int) {
        ...
        val Comic_rank=itemView.findViewById<TextView>(R.id.Comic_rank)
        ...
        Comic_rank.text=(position+1).toString()
...
//            bottomSheetDialog.show()
    ...
    fun addData(rankComicList: RankComicList){
        mData?.addAll(rankComicList.data.returnData.comics)
        notifyDataSetChanged()
    }
    fun Refresh(rankComicList: RankComicList){
        mData?.clear()
        mData?.addAll(rankComicList.data.returnData.comics)
    }
